...
people <- gedcom(subm("Me")) %>% 
  ...
  add_famg(husband = "Implicit dead person 2") %>% 
...
The remaining parameters determine the action to take when living individuals are found. By default, the records are preserved, but all detail is removed, leaving only a change date:
...
The user has the option of leaving a note explaining that the record has been cleansed:

```{r}
remove_living(people, guess = TRUE, add_note = TRUE, note_text = "Record cleansed") %>%
  dplyr::filter(record  == "@I1@")
```
